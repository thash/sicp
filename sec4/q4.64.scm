...
(assert! (rule (outranked-by ?staff-person ?boss)
               (or (supervisor ?staff-person ?boss)
                   (and (outranked-by ?middle-manager ?boss) ;; Louisはここの順番を変更した.
...
;;; Louisの実装
;; -> 上のWarbucks Oliverを出力した後フリーズする(無限ループ).
;; なぜなら, 本来and 1個目のsupervisorで該当なしとなって止まっていたはずの検索がoutranked-byが再帰的に呼ばれ終わらなくなるため.
